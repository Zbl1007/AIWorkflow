...
Browser evidence 的判定标准：

- 必须来自 UI / 浏览器 / 桌面界面交互上下文。
- 必须是机器可检查的事实。
- 必须由 check 验证一个明确断言。
- 必须在 driver 到达稳定采集状态后产生。
- 不能只是 artifact 的存在性。

截图可以参与 browser evidence，但只有在 check 从截图中验证出明确机器事实时才成立；截图文件存在本身不成立。被 check 用作验收依据的产物应通过 check 的 `artifacts` 声明进入 Run 目录。
